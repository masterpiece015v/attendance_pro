...
import tokyo.mstp015v.attendance_pro.databinding.FragmentCheckAttendanceForStudentBinding
...
class CheckAttendanceForStudentFragment : Fragment() {
    class RealmAdapter( data : OrderedRealmCollection<Student>)
        : RealmRecyclerViewAdapter<Student, tokyo.mstp015v.attendance_pro.CheckAttendanceForStudentFragment.RealmAdapter.ViewHolder>(data,true){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val adapter = RealmAdapter(
                ret)
        binding.recyclerCheckAttendanceForStudent.adapter = adapter
        adapter.setOnItemClickListener {
            val action = CheckAttendanceForStudentFragmentDirections.actionToCheckAttendanceFragment(it,args.gName)
